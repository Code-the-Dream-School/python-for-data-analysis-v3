...
1. **[Relational Thinking](<07 - Databases and SQL/01_relational_thinking.md>)**
...
2. **[Creating and Populating a Database](<07 - Databases and SQL/02_creating_populating_databases.md>)**
...
3. **[Querying with SELECT](<07 - Databases and SQL/03_querying_select.md>)**
...
4. **[Joins, Updates, and Deletes](<07 - Databases and SQL/04_joins_updates_deletes.md>)**
...
5. **[SQL and Pandas Together](<07 - Databases and SQL/05_sql_and_pandas.md>)**
...
**1. A student can take many courses, and a course can have many students. How is this represented in a relational database?**

* A) By storing a list of courses in each student's row
* B) With a join table (like `Enrollments`) holding one row per student-course pairing
* C) With a single `UPDATE` statement
* D) It cannot be represented

<details>
<summary>Answer</summary>

B) A many-to-many association uses a join table, because a relational row cannot hold a list. (Module 7.1.)

</details>

**2. Why should you insert values using `?` placeholders rather than building the SQL string with an f-string?**

* A) It is faster to type
* B) Parameterized statements are the safe, reliable way to pass values into SQL
* C) f-strings are not valid Python
* D) It is only a style preference

<details>
<summary>Answer</summary>

B) Parameterized statements pass values safely; building SQL from variables is unsafe. Week 8 explains exactly why. (Module 7.2.)

</details>

**3. You have a large `sales` table in a database and want to analyze only this year's high-value sales in Pandas. What is the better approach?**

* A) Load the whole table into Pandas, then filter
* B) Use SQL (`WHERE`) to select the rows you need, then load that result with `pd.read_sql_query`
* C) Convert the table to CSV first
* D) Use `df.to_sql`

<details>
<summary>Answer</summary>

B) Let the database narrow the data first, then analyze the smaller result in Pandas. (Module 7.5.)

</details>

**4. What does an `UPDATE` or `DELETE` statement with no `WHERE` clause do?**

* A) Nothing — it requires a `WHERE`
* B) It affects every row in the table
* C) It affects only the first row
* D) It raises an error

<details>
<summary>Answer</summary>

B) Without a `WHERE` clause, `UPDATE` changes every row and `DELETE` removes every row. Always confirm the `WHERE` clause first. (Module 7.4.)
